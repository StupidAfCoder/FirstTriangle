...
int main() 
{
	...
	if (window == NULL)
	{
		std::cout << "Not able to create Windows!" << std::endl;
		glfwTerminate();
		return -1;
	}
	...
	Shader myShader("Shaders/vertex3.glsl", "Shaders/fragment3.glsl");
	Shader lightShaders("Shaders/lightvertex.glsl", "Shaders/lightfragment.glsl");
	...
	unsigned char* data = stbi_load("Shaders/container.jpg", &width, &height, &nChannels , 0);
	if (data) 
	{
